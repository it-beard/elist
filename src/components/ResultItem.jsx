 ...
 * кароткая назва, спасылкі і падстава. Фізічная асоба (пералік МУС) — бірузовая палоска і плашка «Асоба · МУС»,
 * імя, транслітарацыя, дата нараджэння з грамадзянствам і статусам, падстава (прысуд). Пазнака спіса вядомая
 * з індэкса яшчэ да загрузкі фрагмента.
 */
export default function ResultItem({ item, tokens, chunkSize }) {
  ...
  return (
    <li className={`item${isF ? ' formation' : ''}${isP ? ' person' : ''}${item.removed ? ' removed' : ''}`}>
      <div className="meta">
        {isP ? (
          <span className="type person" title={t.personTitle}>{t.personLabel}</span>
        ) : isF ? (
