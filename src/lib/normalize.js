 ...
 * дзякуючы гэтаму індэксы супадзенняў у нармалізаваным тэксце
 * можна напрамую выкарыстоўваць для падсветкі ў арыгінале.
 */
const QUOTES = /[«»“”„‟"‘’‚‛']/;
...
export function normalize(text) {
  ...
  for (const ch of text) {
    ...
    if (c.length !== ch.length) c = ch; // рэдкія сімвалы, што мяняюць даўжыню
    if (c === 'ё') c = 'е';
...
/** Кампактная нармалізацыя для індэкса (прабелы сціснутыя). */
...
/** Разбівае запыт на токены; фраза ў лапках — адзін токен. */
...
/**
 * Дыяпазоны [пачатак, канец) супадзенняў токенаў у тэксце (злітыя).
 * Токен можа быць радком або масівам варыянтаў (транслітарацыя, прыблізныя словы).
 */
