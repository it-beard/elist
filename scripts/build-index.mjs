#!/usr/bin/env node
/**
 * data/materials.json + data/formations.json + data/persons.json + data/terror.json + data/wanted.json → public/data/{index.json,
 * chunks/N.json, meta.json}, public/feed.xml і GEO-файлы. Індэкс: кампактныя нармалізаваныя радкі для пошуку
 * (scripts/index-rows.mjs); фрагменты: поўныя запісы, якія браўзер падцягвае ляніва толькі для паказу. Перад гэтым — пералік
 * КДБ уліваецца ў спіс фізічных асоб (foldTerror: хто ёсць і ў пераліку МУС — адной карткай) і лічацца крос-спасылкі паміж
 * асобамі і базай вышуку РФ (linkLists). GEO-факты па гадах лічацца толькі па матэрыялах.
 */
import fs from 'node:fs/promises';
import path from 'node:path';
import { dateWords } from '../src/lib/court.js';
import { DATA_DIR, ROOT, readJson } from './common.mjs';
import { CHUNK, chunkRecord, dict, feed, foldTerror, indexRow, linkLists, publicMeta } from './index-rows.mjs';
...
// астатнія спісы неабавязковыя: без файлаў (свежы клон да першага абнаўлення) сайт працуе як раней
...
const wmeta = await readJson(path.join(DATA_DIR, 'wanted-meta.json'), {});
const terror = await readJson(path.join(DATA_DIR, 'terror.json'), []);
const tmeta = await readJson(path.join(DATA_DIR, 'terror-meta.json'), {});
const db = [...materials.map((x) => ({ ...x, list: 'm' })), ...formations.map((x) => ({ ...x, list: 'f' })), ...persons.map((x) => ({ ...x, list: 'p' })), ...wanted.map((x) => ({ ...x, list: 'w' })), ...terror.map((x) => ({ ...x, list: 't' }))];
const merged = foldTerror(db);
const linked = linkLists(db);
...
const pub = publicMeta(meta, fmeta, pmeta, wmeta, tmeta);
...
console.log(`Індэкс: ${materials.length} матэрыялаў + ${formations.length} фарміраванняў + ${persons.length} фізічных асоб + ${terror.length} з пераліку КДБ (${merged} ёсць і ў пераліку МУС) + ${wanted.length} у вышуку РФ (${linked} звязаных з асобамі), ${(size / 1e6).toFixed(2)} MB, фрагментаў: ${Math.ceil(db.length / CHUNK)}`);
