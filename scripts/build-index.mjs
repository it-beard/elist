#!/usr/bin/env node
/**
 * data/materials.json + data/formations.json + data/persons.json + data/wanted.json → public/data/{index.json, chunks/N.json,
 * meta.json}, public/feed.xml і GEO-файлы. Індэкс: кампактныя нармалізаваныя радкі для пошуку (scripts/index-rows.mjs);
 * фрагменты: поўныя запісы, якія браўзер падцягвае ляніва толькі для паказу. Перад гэтым — крос-спасылкі паміж пералікам
 * фізічных асоб МУС і базай вышуку РФ (linkLists). GEO-факты па гадах лічацца толькі па матэрыялах.
 */
import fs from 'node:fs/promises';
import path from 'node:path';
import { dateWords } from '../src/lib/court.js';
import { DATA_DIR, ROOT, readJson } from './common.mjs';
import { CHUNK, chunkRecord, dict, feed, indexRow, linkLists, publicMeta } from './index-rows.mjs';
...
// другі, трэці і чацвёрты спісы неабавязковыя: без файлаў (свежы клон да першага абнаўлення) сайт працуе як раней
...
const pub = publicMeta(meta, fmeta, pmeta, wmeta);
...
console.log(`Індэкс: ${materials.length} матэрыялаў + ${formations.length} фарміраванняў + ${persons.length} фізічных асоб + ${wanted.length} у вышуку РФ (${linked} звязаных з пералікам МУС), ${(size / 1e6).toFixed(2)} MB, фрагментаў: ${Math.ceil(db.length / CHUNK)}`);
