 ...
 * дата нараджэння і адрас у канал не ідуць — яны на старонцы запісу), 🚫 пералік КДБ «датычных да тэрарыстычнай
...
export const NAME = {
  ...
  p: 'Пералік фізічных асоб, датычных да экстрэмісцкай дзейнасці (МУС)',
  t: 'Пералік асоб, датычных да тэрарыстычнай дзейнасці (КДБ)',
...
export function splitDigest(fresh, { header, footer, site, limit = LIMIT }) {
  ...
  for (const b of blocks) {
    ...
    if (next.length > limit && count > 0) {
      ...
      cur = `<i>Працяг дайджэста</i>\n\n${b.html}`; curIds = [b.id]; count = 1;
    } else { cur = next; curIds.push(b.id); count++; }
  }
  messages.push(`${cur}\n\n${footer}`); ids.push(curIds);
  if (messages.length > 1) messages.forEach((m, i) => { messages[i] = m.replace(/^(<i>Працяг дайджэста<\/i>)/, `<i>Працяг дайджэста (${i + 1}/${messages.length})</i>`); });
  return { messages, ids };
}
