...
function fallback(base, f) {
  return `
      <header class="top wrap">
        ...
        <p class="sub">${f.total ? `${f.formations || f.persons || f.wanted ? [`${esc(f.totalStr)} матэрыялаў`, f.formations && `${esc(f.formationsStr)} фарміраванняў`, f.persons && `${esc(f.personsStr)} асоб`, f.wanted && `${esc(f.wantedStr)} у вышуку РФ`].filter(Boolean).join(' · ') : `${esc(f.totalStr)} запісаў`} · праверана ${esc(f.updatedStr)}` : 'Пошук па афіцыйных спісах'}</p>
      </header>
      <main class="wrap">
        <p class="summary">Загрузка індэкса…</p>
        <noscript>
          <h2>Пошук па Рэспубліканскім спісе экстрэмісцкіх матэрыялаў, пераліку экстрэмісцкіх фарміраванняў, пераліку фізічных асоб Беларусі і базе вышуку РФ па беларусах</h2>
